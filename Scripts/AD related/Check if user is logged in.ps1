...
$NPrefix = "your AD domain prefix*"
# search
$computers = Get-ADComputer -Filter {Enabled -eq 'true' -and SamAccountName -like $NPrefix}
$CompCount = $Computers.Count
Write-Host "Searching for $Username on $NPrefix on $CompCount Computers`n"
                ...
                if($Reply -eq 'True'){
                                ...
                                if([string]::IsNullOrEmpty($proc)){
                                                ...
                                                ForEach ($p in $proc) {                                                   
                                                                ...
                                                                if ($temp -eq $Username){
                                                                write-host "$Username is logged on $Computer" -ForegroundColor Red -BackgroundColor White
                                                                }
                                                }
                                }              
                }
